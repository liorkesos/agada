	...
	$.fn.jsquares = $.fn.jSquares = function(options){
		...
		var main = this,
			...
			defaults = { 
				...
				caption_width: 353, // caption overlay width
				caption_height:210, // caption overlay height
				fade_to: 1, // fade image/caption to what..
				fade_start: 1, // send image back to...
				fade_speed: 1000, // int or string: anytime the fade effect is used, how fast should it be
				shuffle_in: true, // have the pictures all fade in on page load?
				shuffle_in_speed: 100, // how long to wait before we fade in the next image on page load
				...
				caption_slidedown_speed: 300, // how fast to slidedown the caption
		...
		*/
		function _caption(index){
			...
			if(contents){
				...
				contents = wrap_a_start + wrap_a_end;
			...
			if(options.caption_slide_down){
				$('#js-overlay-id-' + index).fadeIn(options.caption_slidedown_speed)
			}
		...
		*/
		...
		//	alert($(image).width())
			var loc_x = loc.x + $(image).width()/2 
			var loc_y = loc.y + $(image).height()/2
